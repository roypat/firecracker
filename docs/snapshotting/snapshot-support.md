...
_Note_: If the files indicated by `snapshot_path` and `mem_file_path` don't
...
_Note_: If not specified, `snapshot_type` is by default `Full`.
...
_Note_: On a fresh microVM, `track_dirty_pages` field should be set to `true`,
...
_Note_: This is an example of an API command that enables dirty page tracking:
...
_Notes_: Please, keep in mind that only by setting to true
