...
class Actions:
    ...
    def put(self, **args):
        ...
        return self._api_session.put("{}".format(self._actions_cfg_url), json=datax)
...
class Balloon:
    ...
    def put(self, **args):
        ...
        datax = self.create_json(**args)
        return self._api_session.put("{}".format(self._balloon_cfg_url), json=datax)

    def patch(self, **args):
        """Update a previously attached balloon device."""
        datax = self.create_json(**args)
        return self._api_session.patch("{}".format(self._balloon_cfg_url), json=datax)

    def patch_stats(self, **args):
        """Update the balloon statistics interval."""
        datax = self.create_json(**args)
        return self._api_session.patch(
            "{}".format(self._balloon_cfg_url + "/statistics"), json=datax
        )

    def get(self):
        """Get the response of specifying the balloon configuration."""
        return self._api_session.get(self._balloon_cfg_url)

    def get_stats(self):
        """Get the response of specifying the balloon statistics."""
        return self._api_session.get("{}".format(self._balloon_cfg_url + "/statistics"))

    @staticmethod
    def create_json(
        amount_mib=None, deflate_on_oom=None, stats_polling_interval_s=None
    ):
        """Compose the json associated to this type of API request."""
        datax = {}
...
class BootSource:
    ...
    def put(self, **args):
        ...
        return self._api_session.put("{}".format(self._boot_cfg_url), json=datax)

    def patch(self, **args):
        """Update a previously attached boot source."""
        datax = self.create_json(**args)
        return self._api_session.patch("{}".format(self._boot_cfg_url), json=datax)

    def get(self):
        """Get the response of specifying a boot source."""
        return self._api_session.get(self._boot_cfg_url)

    @staticmethod
    def create_json(boot_args=None, kernel_image_path=None, initrd_path=None):
        """Compose the json associated to this type of API request."""
        datax = {}
...
class DescribeInstance:
    ...
    def __init__(self, api_usocket_full_name, api_session):
        """Specify the information needed for sending API requests."""
        url_encoded_path = urllib.parse.quote_plus(api_usocket_full_name)
        self._descinst_cfg_url = API_USOCKET_URL_PREFIX + url_encoded_path + "/"
        self._api_session = api_session

    def get(self):
        """Get the status of configuring the current microvm."""
        return self._api_session.get(self._descinst_cfg_url)
...
class Drive:
    ...
    def put(self, **args):
        ...
        return self._api_session.put(
            "{}/{}".format(self._drive_cfg_url, args["drive_id"]), json=datax
        )
    ...
    def get(self, drive_id):
        ...
        return self._api_session.get("{}/{}".format(self._drive_cfg_url, drive_id))
...
class FullConfig:
    ...
    def get(self):
        ...
        return self._api_session.get(self._export_cfg_url)
...
class InstanceVersion:
    ...
    def get(self):
        ...
        return re.match(
            r"^Firecracker v([0-9]+\.[0-9]+(\.[0-9]+)?)", stdout.partition("\n")[0]
        ).group(1)

    def get_from_api(self):
        """Get the version of the current microvm, from the API."""
        return self._api_session.get(self._version_cfg_url)
...
class Logger:
    ...
    def put(self, **args):
        ...
        return self._api_session.put("{}".format(self._logger_cfg_url), json=datax)
...
class SnapshotLoad:
    ...
    def put(self, timeout=None, **args):
        ...
        datax = self.create_json(**args)
        return self._api_session.put(
            "{}".format(self._snapshot_cfg_url), json=datax, timeout=timeout
        )

    @staticmethod
    def create_json(
        snapshot_path, diff=False, resume=False, mem_backend=None, mem_file_path=None
    ):
        """Compose the json associated to this type of API request."""
        if mem_file_path:
            datax = {
...
class Metrics:
    ...
    def put(self, **args):
        ...
        datax = self.create_json(**args)
        return self._api_session.put("{}".format(self._metrics_cfg_url), json=datax)

    def patch(self, **args):
        """Configure or update the settings of the metrics system."""
        datax = self.create_json(**args)
        return self._api_session.patch("{}".format(self._metrics_cfg_url), json=datax)
...
class MachineConfigure:
    ...
    def put(self, **args):
        ...
        return self._api_session.put(
            "{}".format(self._machine_cfg_url), json=self._datax
        )
    ...
    def patch(self, **args):
        ...
        return self._api_session.patch("{}".format(self._machine_cfg_url), json=datax)

    def get(self):
        """Get the status of configuring the current microvm."""
        return self._api_session.get(self._machine_cfg_url)
...
class MMDS:
    ...
    def put(self, **args):
        ...
        return self._api_session.put("{}".format(self._mmds_cfg_url), json=args["json"])

    def put_config(self, **args):
        """Send a new MMDS config request."""
        return self._api_session.put(
            "{}".format(self._mmds_cfg_url + "/config"), json=args["json"]
        )

    def patch(self, **args):
        """Update the details of some MMDS request."""
        return self._api_session.patch(
            "{}".format(self._mmds_cfg_url), json=args["json"]
        )

    def get(self):
        """Get the status of the MMDS request."""
        return self._api_session.get(self._mmds_cfg_url)
...
class Network:
    ...
    def put(self, **args):
        ...
        return self._api_session.put(
            "{}/{}".format(self._net_cfg_url, args["iface_id"]), json=datax
        )
...
class Vm:
    ...
    def patch(self, **args):
        ...
        return self._api_session.patch(self._vm_cfg_url, json=datax)
...
class Vsock:
    ...
    def put(self, **args):
        ...
        return self._api_session.put(self._vsock_cfg_url, json=datax)

    def patch(self, **args):
        """Apply an update to some vsock device."""
        datax = self.create_json(**args)

        return self._api_session.patch(self._vsock_cfg_url, json=datax)

    @staticmethod
    def create_json(guest_cid, uds_path, vsock_id=None):
