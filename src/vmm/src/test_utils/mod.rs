...
pub fn single_region_mem(region_size: usize) -> Memory {
...
pub fn single_region_mem_at(at: u64, size: usize) -> Memory {
    multi_region_mem(&[(GuestAddress(at), size)])
}

/// Creates a [`GuestMemoryMmap`] with multiple regions and without dirty page tracking.
pub fn multi_region_mem(regions: &[(GuestAddress, usize)]) -> Memory {
    Memory::from_raw_regions(regions, false, HugePageConfig::None)
...
pub fn arch_mem(mem_size_bytes: usize) -> Memory {
