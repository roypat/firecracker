...
pub enum VmConfigError {
    ...
    InvalidMemorySize,
    /// The number of vCPUs must be greater than 0, less than {MAX_SUPPORTED_VCPUS:} and number can only be 1 or an even number if SMT is enabled.
    InvalidVcpuCount,
...
pub enum HugePageConfig {
    ...
    #[serde(rename = "1G")]
    Hugetlbfs1G,
    /// Enable transparent huge pages (THP) for guest memory
    Thp,
}
...
impl HugePageConfig {
    ...
    fn is_valid_mem_size(&self, mem_size_mib: usize) -> bool {
        let divisor = match self {
            // THP does not impose any requirements on the memory size, as anything "left over"
            // that does not fit into a huge page will happily continue using 4K pages.
            // Note that any integer memory size expressed in MiB will be a multiple of 4096KiB.
            HugePageConfig::None | HugePageConfig::Thp => 1,
            HugePageConfig::Hugetlbfs2M => 2,
            HugePageConfig::Hugetlbfs1G => 1024,
        };
    ...
    ///
    /// Returns `0` if `self == HugePageConfig::Thp`, as transparent huge pages are enabled
    /// via `madvise` instead of `mmap` flags.
    pub fn mmap_flags(&self) -> libc::c_int {
        match self {
            HugePageConfig::None | HugePageConfig::Thp => 0,
            HugePageConfig::Hugetlbfs2M => libc::MAP_HUGETLB | libc::MAP_HUGE_2MB,
            HugePageConfig::Hugetlbfs1G => libc::MAP_HUGETLB | libc::MAP_HUGE_1GB,
        }
    }
    ...
    pub fn is_hugetlbfs(&self) -> bool {
        matches!(
            self,
            HugePageConfig::Hugetlbfs2M | HugePageConfig::Hugetlbfs1G
        )
    }
}
...
impl From<HugePageConfig> for Option<memfd::HugetlbSize> {
    fn from(value: HugePageConfig) -> Self {
        match value {
            HugePageConfig::None | HugePageConfig::Thp => None,
            HugePageConfig::Hugetlbfs2M => Some(memfd::HugetlbSize::Huge2MB),
            HugePageConfig::Hugetlbfs1G => Some(memfd::HugetlbSize::Huge1GB),
        }
    }
}
...
impl VmConfig {
    ...
    pub fn update(&self, update: &MachineConfigUpdate) -> Result<VmConfig, VmConfigError> {
        ...
        if page_config.is_hugetlbfs() && KernelVersion::get()? < KernelVersion::new(5, 10, 0) {
...
mod tests {
    ...
    fn test_hugetlbfs_not_supported_4_14() {
        if KernelVersion::get().unwrap() < KernelVersion::new(4, 15, 0) {
            let base_config = VmConfig::default();
            let update = MachineConfigUpdate {
                huge_pages: Some(HugePageConfig::Hugetlbfs1G),
