...
output. It the searches for list-valued properties in the EMF, and runs a
...
def extract_metrics(emf):
    """Extracts the cloudwatch metrics from an EMF log message"""
    metrics_description = {
        y["Name"]: y["Unit"] for y in emf["_aws"]["CloudWatchMetrics"][0]["Metrics"]
    }
    return {
        metric_name: (emf[metric_name], unit)
        for metric_name, unit in metrics_description.items()
    }


def reemit_emf_log_entry(log_entry: str, revision: str):
    ...
    dimensions = extract_dimensions(emf)
    metrics = extract_metrics(emf)

    metrics_logger.set_dimensions(dimensions)

    for name, (value, unit) in metrics.items():
        metrics_logger.put_metric(name, value, unit)

    result = {}

    for key in emf:
        if key not in dimensions and key not in metrics:
            metrics_logger.set_property(key, emf[key])

            if isinstance(emf[key], list):
                result[key] = emf[key]

    asyncio.run(metrics_logger.flush())

    return frozenset(dimensions.items()), result
...
def load_data_series(revision: str):
    ...
    its list-valued properties.

    Also reemits all EMF logs using aws-embedded-metrics."""
    data = {}
    ...
    for test in report["tests"]:
        for line in test["teardown"]["stdout"].splitlines():
            ...
            if line.startswith("{"):
                dimensions, result = reemit_emf_log_entry(line, revision)
...
def analyze_data(data_a, data_b):
    ...
    Returns a mapping of dimensions and properties to the result of their regression test.
    """
